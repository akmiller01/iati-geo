...
if __name__ == '__main__':
    ...
    shutil.rmtree("C:/git/iati-geo/sep/")
    os.mkdir("C:/git/iati-geo/sep/")
    
    # Where to look for saved IATI XML via registry-refresher
    rootdir = 'C:/git/iati-geo/data'

    # Predefined header names
    header = iatiflat.header
    full_header = header + ["publisher"]
    header_frame = pd.DataFrame([full_header])
    header_frame.to_csv("C:/git/iati-geo/sep/000header.csv", index=False, header=False, encoding="utf-8")
    ...
    for subdir, dirs, files in os.walk(rootdir):
        for filename in files:
            ...
            if len(output) > 0:
                ...
                data.to_csv("C:/git/iati-geo/sep/{}.csv".format(filename), index=False, header=False, encoding="utf-8")
    ...
    os.system("cat C:/git/iati-geo/sep/*.csv > C:/git/iati-geo/output/iati_unfiltered_agg.csv")